...
try {
    ...
    #
    # 2) Create the OU only if truly absent
    #
    ...
    foreach ($u in $users) {
        #
        # Pick the right field for first+last name:
        #
        $given = 
            $u.'First Name'   `
            -or $u.FirstName  `
        ...
        # build UPN from samAccount if present, otherwise fallback
        if ($u.samAccount) {
            $upn = "$($u.samAccount)@consultingfirm.com"
        }
        else {
